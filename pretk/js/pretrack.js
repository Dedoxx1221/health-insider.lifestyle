...
var initialSrc = "https://track.secretlostprayer.com/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("track.secretlostprayer.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://track.secretlostprayer.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://track.secretlostprayer.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("track.secretlostprayer.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
